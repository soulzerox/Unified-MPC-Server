...
export const UPGRADE_TOOL_CATALOG: readonly UpgradeToolCatalogEntry[] = [
  ...
  execute('git_worktree_spawn', 44, 'Create a confined, ledger-owned Git worktree and enforce its dependency resource view using parent-owned shared runtime/cache policy. Bootstrap opt-out is recovery-only and must use an explicit emergency override. Package-manager lifecycle scripts may execute.', ['git', 'worktree', 'agent', 'dependency', 'bootstrap'], { availability: 'optional', requirements: ['registered Git workspace', 'package manager when dependency bootstrap is supported'], supportsCancel: true, supportsDryRun: true, auditTarget: 'git-worktree' }),
  dangerous('git_worktree_remove', 44, 'Remove a ledger-owned Git worktree after dry-run and standard-mode confirmation; trusted Full Bypass skips unified-mpc approval.', ['git', 'worktree', 'agent'], { availability: 'optional', requirements: ['registered Git workspace', 'ownership ledger entry'], supportsCancel: true, supportsDryRun: true, auditTarget: 'git-worktree' }),
