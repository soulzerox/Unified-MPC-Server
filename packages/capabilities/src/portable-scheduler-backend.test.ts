...
describe('portable scheduler providers', () => {
  ...
  it('rejects control characters before serializing launchd or systemd units', async () => {
    const calls: string[] = [];
    const home = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-scheduler-control-'));
    ...
    try {
      ...
      expect(calls).toHaveLength(0);
      await expect(readFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-newline-command.service'), 'utf8')).rejects.toThrow();
    } finally { await rm(home, { recursive: true, force: true }); }
  });

  it('keeps macOS launchd mutations explicit and escapes plist arguments', async () => {
    const home = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-launchd-'));
    const calls: string[] = [];
    ...
    try {
      ...
      expect(created).toMatchObject({ ok: true, value: { backend: 'launchd', label: 'com.unified-mpc.daily-report' } });
      const plist = await readFile(path.join(home, 'Library', 'LaunchAgents', 'com.unified-mpc.daily-report.plist'), 'utf8');
      expect(plist).toContain('a&amp;b');
      expect(plist).toContain('<key>unified-mpcTaskName</key><string>daily report</string>');
      ...
      expect((await readFile(path.join(home, 'Library', 'LaunchAgents', 'com.unified-mpc.daily-report.plist'), 'utf8'))).toContain('/usr/bin/printf');
      ...
      const weekly = await readFile(path.join(home, 'Library', 'LaunchAgents', 'com.unified-mpc.weekly-report.plist'), 'utf8');
      ...
      const once = await readFile(path.join(home, 'Library', 'LaunchAgents', 'com.unified-mpc.once-report.plist'), 'utf8');
      expect(once).toContain('<key>Year</key>');
      expect(once).toContain('<integer>2030</integer>');
      await writeFile(path.join(home, 'Library', 'LaunchAgents', 'com.unified-mpc.foreign.plist'), '<plist><dict><key>Label</key><string>com.unified-mpc.foreign</string></dict></plist>');
  ...
  it('writes only owned Linux user units and does not fall back to cron', async () => {
    const home = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-systemd-'));
    const calls: string[] = [];
    const backend = new LinuxSchedulerCapabilityBackend({ homeDirectory: home, runImpl: async (executable: string, args: readonly string[]): Promise<{ stdout: string; stderr: string }> => { calls.push(`${executable} ${args.join(' ')}`); return { stdout: '', stderr: '' }; } });
    try {
      ...
      expect(created).toMatchObject({ ok: true, value: { backend: 'systemd-user', service: 'unified-mpc-build-test.service', timer: 'unified-mpc-build-test.timer' } });
      ...
      expect((await readFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-build-test.service'), 'utf8'))).toContain('ExecStart=/usr/bin/node');
      expect((await readFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-build-test.service'), 'utf8'))).toContain('100%% done');
      ...
      const weekly = await readFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-weekly-build.timer'), 'utf8');
      ...
      const once = await readFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-once-build.timer'), 'utf8');
      expect(once).toContain('OnCalendar=2030-04-05 08:00:00');
      await writeFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-foreign.service'), '[Service]\nExecStart=/usr/bin/false\n');
      await writeFile(path.join(home, '.config', 'systemd', 'user', 'unified-mpc-foreign.timer'), '[Timer]\nOnCalendar=hourly\n');
      const foreignListed = await backend.execute({ action: 'list' });
      expect(foreignListed).toMatchObject({ ok: true, value: { tasks: expect.not.arrayContaining([expect.objectContaining({ name: 'unified-mpc-foreign.service' })]) } });
      await expect(backend.execute({ action: 'run', task_name: 'build-test', userConfirmed: true })).resolves.toMatchObject({ ok: true, value: { started: true, timer: 'unified-mpc-build-test.timer' } });
      expect(calls.some((call) => call.includes('systemctl --user start unified-mpc-build-test.service'))).toBe(true);
      expect(calls.some((call) => call.includes('systemctl --user enable --now unified-mpc-build-test.timer'))).toBe(true);
      const listed = await backend.execute({ action: 'list' });
      expect(listed).toMatchObject({ ok: true, value: { available: true, ready: true, tasks: expect.arrayContaining([expect.objectContaining({ name: 'unified-mpc-build-test.service' })]) } });
