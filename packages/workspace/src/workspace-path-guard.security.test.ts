...
describe('WorkspacePathGuard reparse-point security', () => {
  it('rejects a read through a junction that points outside the workspace', async ({ skip }) => {
    ...
    const rootPath = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-junction-root-'));
    const outsidePath = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-junction-outside-'));
