...
| Host approval | With Full Bypass OFF, used when the active profile/action requires confirmation. Full Bypass ON skips the unified-mpc host approval provider. |
| Recoverable | `yes` means unified-mpc owns a pre-image/recovery path before replacement/deletion. `external/unknown` means the remote/native side effect cannot be restored by unified-mpc. |
...
**Covered tools:** `workspace_register`, `workspace_activate`, `workspace_deactivate`, `workspace_set_primary`, `workspace_index`, `workspace_index_watch`, `workspace_index_stop`, `session_handoff`, `verify_incremental`, `recipe_run`, `run_affected_tests`, `cache_clear`, `cache_invalidate`, `hook_register`, `hook_remove`, `plugin_install`, `plugin_enable`, `plugin_disable`, `plugin_remove`, `session_checkpoint`, `ponytail_session`, `rag_remember`, `record_turn`, `project_profile_set`, `tool_schema_register`, `task_create`, `task_cancel`, `delegate`, `delegate_cancel`, `parallel_delegate`, `debug_attach`, `debug_step`, `skills_import`, `skills_install`, `mcp_install`, `agent_swarm_run`.

**Turn persistence:** `record_turn` is a bounded parent-owned write whose destination is fixed to `thai-rag-mcp/remember_turn`; `rag_recall` and `rag_remember` are separately bounded trusted-memory primitives and do not reclassify generic `mcp_call`. Local Cline, OpenCode, and Antigravity adapters are enabled by the CLI runtimes and may read only their supported local transcript stores; they baseline existing completed turns and stage newly completed visible user/assistant turns into a signed journal. The supervisor validates the signature and transcript bounds, resolves the project against registered workspaces, dispatches through the trusted Thai-RAG adapter, ACKs only successful persistence, and leaves failures pending for replay. `task_bootstrap` keeps the persistence contract `required` on all packaged transports; it accepts a caller-supplied stable `turnId` or generates one when the host does not provide it, and a new correlated task remains blocked until the prior turn is persisted. ChatGPT Web differs only in capture mode: its transcript path is policy-assisted because the MCP bridge cannot inspect invisible host conversation text. Before trusted recall or persistence, the parent describes the live Thai-RAG child, rejects workspace-scoped shadow definitions, validates the expected child tools/idempotency contract, and supplies current descriptor/catalog fingerprints. The bounded ledger deduplicates stable turn IDs and releases failed claims for retry. Unknown or mutating generic child calls remain opaque and preserve their normal approval boundary; no path infers invisible ChatGPT Web text.
...
| Mutation kind | Chat confirmation | Host approval | Recoverable | Auto-approvable | Active Project | Command policy | Packaged transports |
| --- | --- | --- | --- | --- | --- | --- | --- |
| read Git subcommands are `read`; ordinary repository changes are replace/opaque; data-loss forms are `delete` or destructive replace | Full ordinary Git mutations do not prompt; destructive forms ask unless an enabled exact-scope Git family applies | only when the action remains approval-required | arbitrary Git mutation is not promised Recovery Trash coverage | exact scoped `git_rm`, `git_clean`, and supported exact restore forms may be auto-approved; broad reset/restore and remote/history deletion are not | workspaceId must match Active Project for mutation; worktrees are restricted to owned `.worktrees`/`.unified-mpc/worktrees` entries | scope overrides/aliases and dangerous remote/history rewrites remain denied; project data-loss forms are confirmation-gated | Full ordinary Git needs no duplicate native prompt; interactive destructive forms use the trusted provider |
...
**Covered tools:** `web_fetch`, `mcp_call`, `working_memory_record`.

`workspace_bootstrap`, default `prepare_code_change`, and `working_memory_search` use curated read-only native adapters over the mandatory child-MCP harness and are inventoried in the read family above. `prepare_code_change` may opt into `runGodkillerSafetyCheck=true`; that parent-owned path describes the live Godkiller child, rejects workspace-scoped definitions and contract drift, and invokes only the fixed `gk_task(action=edit_safe)` operation with live descriptor/catalog fingerprints. This does not reclassify generic `mcp_call` or mark every `gk_task` action read-only. `working_memory_record` is the only curated native child mutation: it is a bounded project work-log write. Mandatory native promotion rejects workspace-scoped MCP definitions, pins only globally/user configured child servers, fingerprints their contracts, and validates the exact required tool names before the harness is marked ready.
