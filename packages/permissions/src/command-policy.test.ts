...
describe('CommandPolicy', () => {
  ...
  it('allows normal shell hosts in Balanced and leaves Safe approval semantics intact', () => {
    expect(policy.decide(permissionProfiles.balanced, 'bash', 'project')).toBe('ALLOW');
    expect(policy.decide(permissionProfiles.balanced, 'sh', 'client')).toBe('ALLOW');
    expect(policy.decide(permissionProfiles.balanced, 'python', 'client')).toBe('ALLOW');
    expect(policy.decide(permissionProfiles.safe, 'bash', 'client')).toBe('ASK');
  });
...
describe('CommandPolicy unrestricted', () => {
  ...
  it('allows shell hosts in unrestricted mode', () => {
    ...
    expect(unrestricted.decide(permissionProfiles.full, 'python3', 'client')).toBe('ALLOW');
  });
