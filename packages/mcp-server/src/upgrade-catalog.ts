...
export const UPGRADE_TOOL_CATALOG: readonly UpgradeToolCatalogEntry[] = [
  ...
  execute('git_worktree_spawn', 44, 'Create a confined, ledger-owned Git worktree and, by default, bootstrap its isolated dependency view using ecosystem-native shared caches/stores. Package-manager lifecycle scripts may execute.', ['git', 'worktree', 'agent', 'dependency', 'bootstrap'], { availability: 'optional', requirements: ['registered Git workspace', 'package manager when dependency bootstrap is supported'], supportsCancel: true, supportsDryRun: true, auditTarget: 'git-worktree' }),
