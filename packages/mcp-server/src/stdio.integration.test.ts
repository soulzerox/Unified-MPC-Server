...
function modernMeta(): Record<string, unknown> {
  return {
    ...
    [CLIENT_INFO_META_KEY]: { name: 'unified-mpc-modern-stdio-wire-test', version: '0.1.0' },
    [CLIENT_CAPABILITIES_META_KEY]: {
...
describe('MCP stdio transport', () => {
  ...
  it('serves independent 2026-07-28 requests with protocol-only stdout', async () => {
    ...
    try {
      ...
      expect(diagnostics()).toContain('unified-mpc-stdio-test-diagnostic');
    } finally {
      await client.close();
    }
  }, 30_000);
  ...
  it('serves the modern tasks extension lifecycle over the real stdio child process', async () => {
    ...
    try {
      ...
      expect(created.taskId).toEqual(expect.stringMatching(/^unified-mpc-task-v1\./));
