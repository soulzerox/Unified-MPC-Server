...
- **Completed-Turn Local RAG Persistence**: `record_turn` is a bounded parent-owned write fixed to `thai-rag-mcp/remember_turn`, while trusted `rag_recall` / `rag_remember` expose the curated memory surface without routing through generic opaque `mcp_call`. The CLI runtimes automatically discover completed transcripts from local Cline, OpenCode, and Antigravity storage, stage them through a signed at-least-once journal, resolve them to registered projects, and persist them idempotently through the verified Thai-RAG child. HTTP/stdio share the same bounded compliance ledger; stdio keeps strict correlated-turn enforcement, while HTTP/Web keeps a policy-assisted `record_turn` fallback because ChatGPT Web does not expose invisible transcript text to the MCP server. The parent verifies the live child contract and refuses workspace-scoped Thai-RAG shadow definitions before trusted persistence; generic mutating `mcp_call` remains separately guarded.
...
- **Mandatory Native Child MCPs**: By default, only `memory` and `thai-rag-mcp` are eagerly connected and pinned. Bootstrap verifies their exact required capabilities and fingerprints the child descriptor/catalog contracts. Workspace-scoped MCP definitions are never promoted into this trusted mandatory set. `godkiller` remains available as an optional on-demand safety analyzer and is not required for workspace readiness.
- **Single-Use Pre-Edit Gate**: Every development-artifact path must pass `prepare_code_change` before mutation. The mandatory gate runs `thai-rag-mcp/pre_edit_context`; successful authorization is consumed after one successful mutation and must be refreshed before another edit. High-risk changes can set `runGodkillerSafetyCheck=true`, which uses a curated parent-owned route to the exact optional `godkiller/gk_task(action=edit_safe)` contract after live fingerprint/drift checks and rejects workspace-scoped Godkiller shadows.
