...
**Covered tools:** `workspace_register`, `workspace_activate`, `workspace_deactivate`, `workspace_set_primary`, `workspace_index`, `workspace_index_watch`, `workspace_index_stop`, `session_handoff`, `verify_incremental`, `recipe_run`, `run_affected_tests`, `cache_clear`, `cache_invalidate`, `hook_register`, `hook_remove`, `plugin_install`, `plugin_enable`, `plugin_disable`, `plugin_remove`, `session_checkpoint`, `ponytail_session`, `rag_remember`, `workspace_memory_record`, `rag_code_index`, `rag_cancel_index`, `rag_forget`, `project_profile_set`, `tool_schema_register`, `task_create`, `task_cancel`, `delegate`, `delegate_cancel`, `parallel_delegate`, `debug_attach`, `debug_step`, `skills_import`, `skills_install`, `mcp_install`, `agent_swarm_run`.

**Native memory:** `rag_recall`, `rag_remember`, and `workspace_memory_record` use parent-owned Thai-RAG with canonical workspace scope. Selective memory never captures ordinary turns automatically. `rag_code_index`, `rag_cancel_index`, and `rag_forget` remain bounded mutations; generic `mcp_call` remains opaque and independently approval-guarded.
...
`workspace_bootstrap`, default `prepare_code_change`, and `working_memory_search` use curated read-only native adapters over canonical parent-owned capabilities and are inventoried in the read family above. Required native capability checks fail closed when unavailable. `prepare_code_change` may opt into `runGodkillerSafetyCheck=true`; that parent-owned path describes the optional live Godkiller child, rejects workspace-scoped definitions and contract drift, and invokes only the fixed `gk_task(action=edit_safe)` operation with live descriptor/catalog fingerprints. This does not reclassify generic `mcp_call` or mark every `gk_task` action read-only. `working_memory_record` is the only curated native work-log mutation: it is a bounded project work-log write. Optional child MCP connections may be inspected or used only through their explicit policy and verified contracts; they cannot replace canonical native tools or required capabilities.

| Mutation kind | Chat confirmation | Host approval | Recoverable | Auto-approvable | Active Project | Command policy | Packaged transports |
| --- | --- | --- | --- | --- | --- | --- | --- |
| HTTP GET/HEAD and optional child-MCP discovery are `read`; HTTP POST is `opaque`, PUT is `replace`, DELETE is `delete`; `mcp_call` is action-classified/opaque when child effects cannot be proven | Full ordinary remote mutation does not prompt; DELETE or other detected data-loss remains interactive | only when the action requires approval | external/unknown | no remote destructive auto-approval | workspace metadata cannot widen host scope; optional child calls cannot inherit/escalate parent approval | child execution remains subject to the child/tool policy; no parent privilege escalation | profile-aware central registry; approval-required remote actions and unavailable required capabilities fail closed without a trusted path |
