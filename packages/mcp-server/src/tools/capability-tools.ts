...
import { DEFAULT_MCP_POLL_WAIT_SECONDS, MAX_CONFIGURABLE_WAIT_SECONDS, MIN_CONFIGURABLE_WAIT_SECONDS } from '@unified-mpc/shared';
...
export function capabilityTools(context: McpToolContext, setOfMarksStore?: SetOfMarksObservationStore): McpToolDefinition[] {
  ...
  return [
    ...
    defineTool({
      name: 'health',
      description: 'Diagnostics only. Check all unified-mpc backends or one public tool after a failure, when asked for status, or while diagnosing permissions. Do not use as a preflight before normal work.',
      permission: 'READ',
      annotations: { readOnlyHint: true, destructiveHint: false },
    ...
    defineTool({
      name: 'web_fetch',
      description: 'Fetch an http/https URL (GET/POST/PUT/DELETE/HEAD) with bounded size and timeout. In standard mode every POST, PUT, or DELETE requires explicit chat confirmation and host approval; trusted Full Bypass skips unified-mpc approval. dry_run remains safe. Returns status, headers, and text or base64 body.',
      permission: 'READ',
      annotations: { readOnlyHint: false, destructiveHint: true },
    ...
    defineTool({
      name: 'audio',
      description: 'Record the microphone to a WAV file or play a local audio file through the host media provider. In standard mode recording requires the host-selected Active Project workspaceId and explicit confirmation; trusted Full Bypass skips unified-mpc approval/scope checks. Existing in-workspace outputs use Recovery Trash before replacement when available. record is synchronous and limited to 600 seconds. Use stop to abort an ongoing record/play.',
      permission: 'EXECUTE',
      annotations: { readOnlyHint: false, destructiveHint: true },
    ...
    defineTool({
      name: 'screen_record',
      description: 'Record the screen to an MP4 using the host-native media provider. In standard mode starting a recording requires the host-selected Active Project workspaceId and explicit confirmation; trusted Full Bypass skips unified-mpc approval/scope checks. Existing in-workspace outputs use Recovery Trash before replacement when available. start spawns a background capture, status checks it, stop finalizes the file. Recording stops automatically after 3600 seconds.',
      permission: 'EXECUTE',
      annotations: { readOnlyHint: false, destructiveHint: true },
    ...
    defineTool({
      name: 'office',
      description: 'Automate a supported spreadsheet/document provider when installed. In standard mode every write, replace, merge, or save_as action requires an Active Project workspaceId, explicit chat confirmation, and host approval. Trusted Full Bypass skips unified-mpc approval/scope checks without forging userConfirmed. Existing in-workspace targets use Recovery Trash before replacement when available. Unsupported app/action pairs are dependency-gated rather than approximated.',
      permission: 'WRITE',
      annotations: { readOnlyHint: false, destructiveHint: false },
