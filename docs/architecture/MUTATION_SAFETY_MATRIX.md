...
**Covered tools:** `workspace_register`, `workspace_activate`, `workspace_deactivate`, `workspace_set_primary`, `workspace_index`, `workspace_index_watch`, `workspace_index_stop`, `session_handoff`, `verify_incremental`, `recipe_run`, `run_affected_tests`, `cache_clear`, `cache_invalidate`, `hook_register`, `hook_remove`, `plugin_install`, `plugin_enable`, `plugin_disable`, `plugin_remove`, `session_checkpoint`, `ponytail_session`, `record_turn`, `project_profile_set`, `tool_schema_register`, `task_create`, `task_cancel`, `delegate`, `delegate_cancel`, `parallel_delegate`, `debug_attach`, `debug_step`, `skills_import`, `skills_install`, `mcp_install`, `agent_swarm_run`.

**Turn persistence:** `record_turn` is a bounded parent-owned write whose destination is fixed to `thai-rag-mcp/remember_turn`; callers provide only bounded transcript metadata and cannot select an arbitrary child server or child tool. `task_bootstrap` can correlate the task with a stable `turnId`: stdio defaults to strict enforcement (missing IDs are rejected and the next correlated task is blocked while the previous turn is unpersisted), while HTTP/Web defaults to best-effort violation reporting so a host that cannot expose completed transcript text remains usable. Before dispatch, the parent describes the live Thai-RAG child, rejects workspace-scoped shadow definitions, and supplies the current descriptor/catalog fingerprints on every persistence call. A transport-scoped bounded ledger deduplicates a stable `turnId` per user/assistant role across per-request registry recreation and releases failed claims so a later retry is possible; a successful `record_turn` closes the correlated turn. This does not reclassify generic `mcp_call`: unknown or mutating child calls remain opaque and preserve their normal approval boundary. The host must still provide the transcript content; the MCP server does not infer invisible chat text on its own.
