...
### 3.8 การยืนยัน Exact-Action สำหรับ Local STDIO / CLI / IDE
...
HTTP/Web จะ **ไม่รับ direct trusted-host approval adapter จาก upstream** และ `userConfirmed: true` ก็ยังไม่สามารถแทนการยืนยันจาก host ได้เหมือนเดิม แต่ packaged MCP HTTP/Web composition จะ inject เฉพาะ **cross-client Host Approval Broker** ที่ local เท่านั้น เมื่อมี trusted IDE/CLI STDIO session online คำขอจาก ChatGPT Web ที่ต้องใช้ host approval จะถูก route ไปยัง worker ของ session นั้นเพื่อแสดง OS dialog/TTY approval แล้วส่งผลกลับ Web; ถ้าไม่มี trusted worker, worker หลุด, หมดเวลา หรือ contract ใช้งานไม่ได้ ระบบจะ deny แบบ fail-closed เมื่อ trusted session ปิด worker จะถูกถอนและ session-scoped approval grant จะหมดทันที ส่วน generic/providerless HTTP ที่ไม่ได้ compose broker ยังถูกปฏิเสธตามเดิม Custom non-Web host ยังสามารถส่ง `hostMutationApprovalProvider` ของตัวเองหรือใช้ `createTrustedHostMutationApprovalProvider` ที่ export จาก `@unified-mpc/mcp-server` ได้
...
3. Runtime จะเชื่อมและ pin native `memory` พร้อมเริ่ม parent-owned native Thai-RAG provider โดยตรวจ handshake ของ provider (version, fingerprint, capability, `workspace_id`, health และ generation) แทนการผูกกับ schema ของ tool ใด tool หนึ่ง ส่วน `godkiller` จะไม่เป็น dependency ของ bootstrap และจะถูกเรียกแบบ on-demand เมื่องานมีความเสี่ยงหรือ blast radius สูง
4. MCP ที่มาจากไฟล์ใน workspace เช่น `.cursor/mcp.json` จะไม่สามารถปลอมชื่อมาทับ mandatory native MCP ได้
5. ก่อนแก้ development artifact แต่ละ path ต้องเรียก `prepare_code_change`; mandatory preflight จะรัน native Thai-RAG `pre_edit_context` หากเป็น refactor ใหญ่, migration, งาน security-sensitive หรือ blast radius ยังไม่ชัด ให้ตั้ง `runGodkillerSafetyCheck=true` เพื่อเพิ่ม curated `godkiller/gk_task(action=edit_safe)` โดย parent จะตรวจว่าไม่ใช่ workspace-scoped shadow และ contract ไม่ drift ก่อนเรียก
