...
export function isProtectedCriticalPath(inputPath: string): boolean {
  ...
  if (parts.includes('.git') || parts.includes('.unified-mpc-recovery') || parts.includes('.unified-mpc-trash') || parts.includes('.lnwjud-recovery') || parts.includes('.lnwjud-trash')) return true;
