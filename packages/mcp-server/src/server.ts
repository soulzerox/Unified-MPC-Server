...
export const MCP_OUTCOME_DRIVEN_INSTRUCTIONS = [
  ...
  'At the start of every user task, call task_bootstrap. It resolves policy_snapshot and loads ask-matt in one server-side read; then follow that skill and use policy-listed native capabilities or child MCP servers when relevant without waiting for the user to name them. Native memory is selective: recall when prior context matters and remember only durable decisions, constraints, preferences, or explicit user-requested memory; never persist ordinary turns automatically.',
  'Use the first-party rag_* and workspace_memory_record tools for native workspace-scoped memory and code retrieval. Do not route native Thai-RAG through generic mcp_call or create automatic per-turn memory writes.',
