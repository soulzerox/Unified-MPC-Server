...
const PRIMITIVE_SEARCH_ENTRIES: readonly SearchCatalogEntry[] = [
  ...
  primitiveEntry('workspace_bootstrap', 'Load and fingerprint the workspace engineering harness and verify required native capabilities before code mutation.', 'READ', ['workspace', 'harness', 'bootstrap', 'code', 'read']),
  ...
  primitiveEntry('skills_install', 'Install a validated local or HTTPS Git Agent Skill into the canonical parent-owned Unified MCP skill store.', 'WRITE', ['skills', 'install', 'git', 'url', 'unified-mpc']),
  primitiveEntry('mcp_install', 'Install or register a child MCP server in the canonical parent-owned Unified MCP registry; stdio accepts a command or HTTPS Git source and HTTP/SSE accept endpoint URLs.', 'WRITE', ['mcp', 'install', 'git', 'url', 'gateway', 'unified-mpc']),
...
export class UpgradeRuntimeService {
  ...
  public async execute(name: string, input: Record<string, unknown>, signal?: AbortSignal, authorization?: InvocationAuthorization): Promise<Result<unknown>> {
    ...
    switch (name) {
      ...
      case 'sandbox_exec':
        return ok(truthfulUnavailable(name, 'unsupported', ['Linux Ubuntu host does not support Windows Sandbox']));
      case 'db_inspect':
  ...
  private async externalMcpInsight(name: 'mcp_discover' | 'mcp_health'): Promise<Result<unknown>> {
    ...
    return ok({
      ...
      runtimePolicy: {
        version: 2,
        approval: { mode: 'trusted-memory-only', safeInternalMemoryOps: 'auto', arbitraryChildTools: 'normal-policy' },
        nativeCapabilities: { available: nativeCapabilities, workspaceScoped: true, selectiveMemory: true, automaticTurnCapture: false },
        externalMcp: { connected: servers.filter((server) => server.connected).length, enabled: servers.filter((server) => server.enabled).length },
      },
    });
  }
