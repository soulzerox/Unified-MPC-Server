...
**Turn persistence:** `record_turn` is a bounded parent-owned write whose destination is fixed to `thai-rag-mcp/remember_turn`; callers provide only bounded transcript metadata and cannot select an arbitrary child server or child tool. Before dispatch, the parent describes the live Thai-RAG child, rejects workspace-scoped shadow definitions, and supplies the current descriptor/catalog fingerprints on every persistence call. A transport-scoped bounded ledger deduplicates a stable `turnId` per user/assistant role across per-request registry recreation and releases failed claims so a later retry is possible. This does not reclassify generic `mcp_call`: unknown or mutating child calls remain opaque and preserve their normal approval boundary. The host must still provide the transcript content; the MCP server does not infer invisible chat text on its own.
