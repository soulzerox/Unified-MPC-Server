...
| Unsupported checkpoint envelope could be quarantined/replaced | `CheckpointKeyStore` now fails closed and leaves foreign or malformed key data untouched; quarantine option and behavior removed. | `packages/storage/src/checkpoint-key-store.ts`; `packages/storage/src/checkpoint-key-store.test.ts` |
...
- Checkpoint key fail-closed behavior is covered by tracked tests in `packages/storage/src/checkpoint-key-store.test.ts`; no temporary `/tmp` repro is used as release evidence.
