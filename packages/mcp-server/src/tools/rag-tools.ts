...
export function ragTools(context: McpToolContext): McpToolDefinition[] {
  return [
    ...
    defineTool({
      ...
      handler: async (input, signal, _authorization, budget) => context.ragRemember === undefined
        ? missingService()
        : context.ragRemember(input.workspaceId, input.content, input.category, signal, budget),
    }),
    defineTool({
    ...
    defineTool({
      ...
      handler: async (input, signal, _authorization, budget) => context.nativeRagCall === undefined
        ? missingService()
        : context.nativeRagCall(input.workspaceId, 'forget', { memory_id: input.memoryId }, signal, budget),
    }),
    defineTool({
