...
import { mcpCallSchema, mcpDescribeSchema, mcpListSchema, policySnapshotSchema, taskBootstrapSchema } from './schemas.js';
...
export function mcpBridgeTools(context: McpToolContext): McpToolDefinition[] {
  return [
    defineTool({
      name: 'task_bootstrap',
      description: 'Resolve the live runtime policy and load the mandatory session-start routing skill (normally ask-matt) in one read-only call. Task bootstrap is independent from selective memory writes.',
      ...readOnlyInspection,
