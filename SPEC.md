...
3. As a developer switching between Cursor, OpenCode, and Antigravity, I want a single sync command to compile and distribute my user-edited P1–Pn runtime policy to all IDE rule files simultaneously, so that agent behavior and execution order remain uniform across editors without coupling policy identity to its current position.
...
- **Cross-Client Policy Synchronizer (`IdeSyncService`)**: Compiles an ordered, user-editable P1–Pn runtime policy into atomic client rule blocks. Semantic policy IDs remain stable while P-positions are derived from the persisted array order; the same live order is exposed through `policy_snapshot` and synchronized across clients.
...
- **Command Dispatcher**:
  ...
  - `unified-mpc sync [--targets <t...>]`: Compiles and pushes the persisted P1–Pn runtime policy order to all connected IDE configuration files simultaneously.
...
- **Completed-Turn Local RAG Persistence**:
  ...
  - `task_bootstrap` accepts a stable `turnId` and registers a transport-scoped compliance state. Stdio defaults to `required`: a missing turn ID is rejected and a new correlated turn is blocked until the prior turn is persisted. HTTP/Web defaults to `best_effort`: an unpersisted prior correlated turn is reported as a violation but does not block the next task because the remote host may not expose completed transcript text.
  ...
  - A transport-scoped bounded idempotency/compliance ledger deduplicates one stable `turnId` per user/assistant role across request-scoped MCP server recreation. Failed or cancelled persistence releases the write claim so the host can retry safely; successful `record_turn` closes the correlated turn state.
