...
export function upgradeTools(context: McpToolContext, incrementalVerifier: IncrementalVerifier, activityTracker?: ActivityTracker): McpToolDefinition[] {
  const runtime = new UpgradeRuntimeService(
    context.services,
    context.actor,
    context.contextEconomy,
    context.isToolExposed,
    incrementalVerifier,
    activityTracker,
    context.discoveryTools,
    {
      ...(context.resourceAdmissionController === undefined ? {} : { resourceAdmissionController: context.resourceAdmissionController }),
      ...(context.resourceAdmissionSessionId === undefined ? {} : { resourceAdmissionSessionId: context.resourceAdmissionSessionId }),
      ...(context.lspProcessAdmissionCost === undefined ? {} : { lspProcessAdmissionCost: context.lspProcessAdmissionCost }),
    },
  );
