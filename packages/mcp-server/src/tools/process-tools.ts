import type { CommandSpec } from '@unified-mpc/domain';
...
export function processTools(context: McpToolContext): McpToolDefinition[] {
  return [
    ...
    defineTool({
      name: 'process_stop',
      description: 'Stop an owned managed process tree after explicit chat confirmation in standard mode. Trusted Full Bypass skips the unified-mpc confirmation gate; exact process ownership still applies.',
      permission: 'EXECUTE',
      annotations: { readOnlyHint: false, destructiveHint: false },
...
function projectCommandTools(context: McpToolContext): McpToolDefinition[] {
  ...
  return definitions.map(({ name, kind }) => defineTool({
    name,
    description: `Immediate-return launcher for the detected project ${kind} command. In standard mode the gateway previews the exact executable/argv for host approval and re-resolves it immediately before spawn; any change requires fresh approval. Trusted Full Bypass skips the unified-mpc approval boundary. Project-owned script bodies remain opaque and are not covered by Recovery Trash.`,
    permission: 'EXECUTE',
    annotations: { readOnlyHint: false, destructiveHint: false },
...
function readApprovedProjectCommand(input: unknown): CommandSpec | undefined {
  if (typeof input !== 'object' || input === null || !('__unifiedMpcApprovedProjectCommand' in input)) return undefined;
  const value = (input as { __unifiedMpcApprovedProjectCommand?: unknown }).__unifiedMpcApprovedProjectCommand;
