...
export function codexTools(context: McpToolContext): McpToolDefinition[] {
  return [
    ...
    defineTool({
      name: 'codex_run',
      description: 'Delegate an instruction to the local Codex CLI in the Active Project. Starting Codex requires explicit chat confirmation and host approval in standard mode; trusted Full Bypass skips those unified-mpc application checks without forging userConfirmed.',
      permission: 'EXECUTE',
    ...
    defineTool({
      name: 'codex_stop',
      description: 'Stop an owned Codex task process after explicit chat confirmation in standard mode. Trusted Full Bypass skips the unified-mpc confirmation gate; task ownership still applies.',
      permission: 'EXECUTE',
      annotations: { readOnlyHint: false, destructiveHint: false },
