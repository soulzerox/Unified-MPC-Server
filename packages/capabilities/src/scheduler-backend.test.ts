...
describe('SchedulerCapabilityBackend', () => {
  ...
  it('creates a task with a quoted command line', async () => {
    ...
    expect(result).toMatchObject({ ok: true, value: { created: true, task_name: 'UnifiedMpcTest' } });
    expect(runImpl).toHaveBeenCalledWith('schtasks.exe', [
      '/Create', '/TN', 'UnifiedMpcTest',
  ...
  it('requires confirmation before deleting a scheduled task', async () => {
    ...
    await expect(backend.execute({ action: 'delete', task_name: 'UnifiedMpcTest' }))
      .resolves.toMatchObject({ ok: false, error: { code: 'PERMISSION_REQUIRED' } });
    expect(runImpl).not.toHaveBeenCalled();

    await expect(backend.execute({ action: 'delete', task_name: 'UnifiedMpcTest', userConfirmed: true }))
      .resolves.toMatchObject({ ok: true, value: { deleted: true, task_name: 'UnifiedMpcTest' } });
    expect(runImpl).toHaveBeenCalledWith('schtasks.exe', ['/Delete', '/TN', 'UnifiedMpcTest', '/F']);
  });
  ...
  it('accepts trusted Full Bypass authorization without caller confirmation', async () => {
    ...
    await expect(backend.execute({ action: 'delete', task_name: 'UnifiedMpcTest' }, undefined, authorization))
  ...
  it('previews a deletion without confirmation or schtasks side effects', async () => {
    ...
    await expect(backend.execute({ action: 'delete', task_name: 'UnifiedMpcTest', dry_run: true }))
      .resolves.toMatchObject({ ok: true, value: { dry_run: true, action: 'delete', task_name: 'UnifiedMpcTest' } });
    expect(runImpl).not.toHaveBeenCalled();
  });
  ...
  it.each(['create', 'run'] as const)('requires confirmation before %s', async (action) => {
    ...
    const input = action === 'create'
      ? { action, task_name: 'UnifiedMpcTest', command: 'tool.exe' }
      : { action, task_name: 'UnifiedMpcTest' };
  ...
  it('warns that a failed mutation may already have completed and never retries schtasks automatically', async () => {
    ...
    const result = await backend.execute({ action: 'delete', task_name: 'UnifiedMpcTest', userConfirmed: true });
  ...
  it('does not invoke schtasks when the caller is already cancelled', async () => {
    ...
    await expect(backend.execute({ action: 'run', task_name: 'UnifiedMpcTest' }, controller.signal))
