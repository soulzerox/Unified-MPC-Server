...
describe('upgrade runtime readiness facades', () => {
  ...
  it('honors an injected portable event-log runner instead of querying the test host', async () => {
    const services = {
      platform: 'linux',
      eventLogRuntimeOptions: {
        portableRunner: async () => ok(JSON.stringify({
          MESSAGE: 'injected-contract-event',
          SYSLOG_IDENTIFIER: 'unified-mpc-test',
        })),
      },
