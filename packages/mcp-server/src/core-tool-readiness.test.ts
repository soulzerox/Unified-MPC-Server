...
describe('core tool readiness', () => {
  it('tracks one representative contract for every core tool in the complete 246-tool inventory', () => {
    const registry = coreRegistry();
    expect(registry.listAll()).toHaveLength(246);
    ...
    expect(coreToolNames(registry)).toHaveLength(108);
  ...
  it('keeps the exhaustive success matrix aligned with all 108 core tools', () => {
    const registry = coreRegistry();
