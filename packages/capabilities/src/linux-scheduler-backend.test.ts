...
describe('LinuxSchedulerCapabilityBackend', () => {
  it('keeps the Linux entry point bound to user systemd and argv-only execution', async () => {
    const calls: string[] = [];
    const home = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-linux-scheduler-'));
    ...
    try {
      await expect(backend.execute({ action: 'create', task_name: 'linux test', command: '/usr/bin/true', userConfirmed: true })).resolves.toMatchObject({ ok: true, value: { backend: 'systemd-user', service: 'unified-mpc-linux-test.service', timer: 'unified-mpc-linux-test.timer' } });
