...
describe('MacosSchedulerCapabilityBackend', () => {
  it('keeps the macOS entry point bound to launchd and the owned LaunchAgents scope', async () => {
    const calls: string[] = [];
    const home = await mkdtemp(path.join(os.tmpdir(), 'unified-mpc-macos-scheduler-'));
    ...
    try {
      await expect(backend.execute({ action: 'create', task_name: 'mac test', command: '/usr/bin/true', userConfirmed: true })).resolves.toMatchObject({ ok: true, value: { backend: 'launchd', label: 'com.unified-mpc.mac-test' } });
