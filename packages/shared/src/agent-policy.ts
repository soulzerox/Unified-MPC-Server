...
export function isProtectedCriticalPath(inputPath: string): boolean {
  ...
  if (parts.includes('.git') || parts.includes('.unified-mpc-recovery') || parts.includes('.unified-mpc-trash') || parts.includes('.unified-mpc-recovery') || parts.includes('.unified-mpc-trash')) return true;
